...
const description = 'Export your QR code as SVG, PNG, or JPEG. Live examples for the download() ref method with custom size and filename options.'
usePageSeo('Download', description)
defineOgImage('DocsCover', { title: 'Download', description })
...
<template>
  <div class="prose">
    ...
    <h1>Download</h1>
    <p>
      Use a template ref to access the <code>download()</code> method, which supports
      SVG, PNG, and JPEG output at any target size.
    </p>
    ...
    <h2>Code</h2>
    <DocsCodeBlock lang="vue" :code="`<script setup lang=&quot;ts&quot;>
...
<template>
  ...
  <button @click=&quot;qr?.download({ format: 'png', size: 1200, name: 'my-qr' })&quot;>
    ...
    <DocsPropsTable
      :rows="[
        { name: 'format', type: '\'svg\' | \'png\' | \'jpeg\'', default: '\'svg\'', description: 'Output file format. SVG is vector; PNG and JPEG are raster.' },
        { name: 'size', type: 'number', default: '500', description: 'Output dimension in pixels (raster formats). SVG ignores this.' },
        { name: 'name', type: 'string', default: '\'qrcode-nuxt\'', description: 'Filename without extension.' },
      ]"
    />

    <div class="callout callout-info">
      SVG downloads preserve your gradients and styles perfectly at any print size.
      PNG/JPEG are useful when the receiving platform does not support SVG.
    </div>
  </div>
</template>
...
.demo-preview { display: flex; align-items: center; justify-content: center; }
.demo-actions { display: flex; flex-direction: column; gap: 10px; align-self: flex-end; }
.demo-label { font-size: 12px; font-weight: 600; letter-spacing: .06em; text-transform: uppercase; color: var(--muted); margin-bottom: 4px; }
.demo-btns { display: flex; flex-direction: row; flex-wrap: wrap; gap: 10px; }
.dl-btn {
